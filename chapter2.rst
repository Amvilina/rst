...
^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  - Проверим теперь фильтрацию с помощью ``tll-read``. Перед нами стоит задача выбрать все сообщения ( из 3-х возможных, в нашем случае ), для которых ``value < 5``. Для этого воспользуемся командой ``/comtest$ tll-read output.dat --filter 'seq < 3 and data.value < 5'``
