...
^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  - Посмотрим на первые 3 сообщения: ``/comtest$ tll-read output.dat --seq 0:2``
  ...
  - Если посмотреть на следующие 3 сообщения ``/comtest$ tll-read output.dat --seq 3:5``, то они будут содержать такие же данные, только отличаться полем ``seq``

  - Проверим теперь фильтрацию с помощью ``tll-read``. Перед нами стоит задача выбрать все сообщения ( из 3-х возможных, в нашем случае ), для которых ``value < 5``. Для этого воспользуемся командой ``/comtest$ tll-read output.dat --filter 'seq < 3 and data.value < 5'``. После ``--filter`` мы должны в кавычках написать логическое выражение на языке ``Lua``, которое возвращает ``true`` ( печатать сообщение ) или ``false`` ( пропуск сообщения )
