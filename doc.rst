...
^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  - Посмотрим на первые 3 сообщения: ``tll-read output.dat -s 0:2``
  ...
  - Если посмотреть на следующие 3 сообщения ``tll-read output.dat -s 3:5``, то они будут содержать такие же данные, только отличаться полем ``seq``
